...
* BUGFIX: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): align request body buffering flags - `maxRequestBodySizeToRetry` and `requestBufferSize` to the same `16KB` value. Disable request buffer if `0` is set for any of those flags. See [#10675](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10675)
